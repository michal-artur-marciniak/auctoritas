...
public class DefaultMicrosoftOAuthClient implements MicrosoftOAuthClient {
  ...
  public MicrosoftTokenResponse exchangeAuthorizationCode(MicrosoftTokenExchangeRequest request) {
    if (request == null) {
      throw new DomainValidationException("oauth_microsoft_exchange_failed");
    }
    ...
    try {
      ...
      if (response == null || response.accessToken() == null || response.accessToken().isBlank()) {
        throw new DomainExternalServiceException("oauth_microsoft_exchange_failed");
      }
      return response;
    } catch (RestClientException ex) {
      throw new DomainExternalServiceException("oauth_microsoft_exchange_failed", ex);
    }
  }
  ...
  public MicrosoftUserInfo fetchUserInfo(String accessToken) {
    ...
    try {
      ...
      if (info == null || info.sub() == null || info.sub().isBlank()) {
        throw new DomainExternalServiceException("oauth_microsoft_userinfo_failed");
      }
      return info;
    } catch (RestClientException ex) {
      throw new DomainExternalServiceException("oauth_microsoft_userinfo_failed", ex);
    }
  }
  ...
  private String decryptCodeVerifier(String codeVerifier) {
    ...
    try {
      ...
      throw new DomainValidationException("oauth_microsoft_exchange_failed", ex);
    }
  }
