...
public class FacebookOAuthProvider implements OAuthProvider {
  ...
  public OAuthAuthorizeDetails getAuthorizeDetails(ProjectSettings settings) {
    ...
    if (!(facebookObj instanceof Map<?, ?> facebookRaw)) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "oauth_facebook_not_configured");
    }
  ...
  public String buildAuthorizeUrl(OAuthAuthorizeDetails details, OAuthAuthorizeUrlRequest request) {
    ...
    if (request == null) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "oauth_facebook_authorize_failed");
    }
