...
@Component
public class MicrosoftOAuthAuthorizeGatewayFilterFactory
    extends AbstractGatewayFilterFactory<MicrosoftOAuthAuthorizeGatewayFilterFactory.Config> {
  private static final String CALLBACK_PATH = "/api/v1/auth/oauth/microsoft/callback";
  ...
  public MicrosoftOAuthAuthorizeGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalMicrosoftAuthorizeRequest request =
          new InternalMicrosoftAuthorizeRequest(projectId, appRedirectUri, state, codeVerifier);

      return webClient
          .post()
          .uri("/internal/oauth/microsoft/authorize")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalMicrosoftAuthorizeResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null
                                || body.clientId() == null
                                || body.clientId().isBlank()
                                || body.authorizationEndpoint() == null
                                || body.authorizationEndpoint().isBlank()
                                || body.scope() == null
                                || body.scope().isBlank()) {
                              return writeErrorResponse(
                                  exchange, HttpStatus.BAD_GATEWAY, "oauth_microsoft_authorize_failed");
                            }
                            String microsoftAuthorizeUrl =
                                UriComponentsBuilder.fromUriString(body.authorizationEndpoint())
                                    .queryParam("client_id", body.clientId())
                                    .queryParam("redirect_uri", callbackUri)
                                    .queryParam("response_type", "code")
                                    .queryParam("scope", body.scope())
                                    ...
                                    .encode()
                                    .toUriString();
                            return redirect(exchange, microsoftAuthorizeUrl);
                          });
                }
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_microsoft_authorize_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
                                ...
                                err == null || err.error() == null || err.error().isBlank()
                                    ? "oauth_microsoft_authorize_failed"
                                    : err.error()));
              })
          .onErrorResume(
  ...
  private record InternalMicrosoftAuthorizeRequest(
      UUID projectId, String redirectUri, String state, String codeVerifier) {}

  private record InternalMicrosoftAuthorizeResponse(
