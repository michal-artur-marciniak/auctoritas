...
@Component
public class GoogleOAuthCallbackGatewayFilterFactory
    extends AbstractGatewayFilterFactory<GoogleOAuthCallbackGatewayFilterFactory.Config> {
  private static final String CALLBACK_PATH = "/api/v1/auth/oauth/google/callback";

  private final WebClient webClient;

  public GoogleOAuthCallbackGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalGoogleCallbackRequest request =
          new InternalGoogleCallbackRequest(code.trim(), state.trim(), callbackUri);

      return webClient
          .post()
          .uri("/internal/oauth/google/callback")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalGoogleCallbackResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null
                                || body.redirectUrl() == null
                                || body.redirectUrl().isBlank()) {
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_google_callback_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
  ...
  private record InternalGoogleCallbackRequest(String code, String state, String callbackUri) {}

  private record InternalGoogleCallbackResponse(String redirectUrl) {}
