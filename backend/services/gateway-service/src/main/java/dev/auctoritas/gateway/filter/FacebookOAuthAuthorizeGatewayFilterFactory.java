...
@Component
public class FacebookOAuthAuthorizeGatewayFilterFactory
    extends AbstractGatewayFilterFactory<FacebookOAuthAuthorizeGatewayFilterFactory.Config> {
  private static final String CALLBACK_PATH = "/api/v1/auth/oauth/facebook/callback";
  private static final String PROJECT_ID_HEADER = "X-Project-Id";

  private static final int STATE_BYTES = 32;
  private static final int CODE_VERIFIER_BYTES = 48;

  private static final String FACEBOOK_AUTHORIZE_URL = "https://www.facebook.com/v18.0/dialog/oauth";
  ...
  public FacebookOAuthAuthorizeGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalFacebookAuthorizeRequest request =
          new InternalFacebookAuthorizeRequest(projectId, appRedirectUri, state, codeVerifier);

      return webClient
          .post()
          .uri("/internal/oauth/facebook/authorize")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalFacebookAuthorizeResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null || body.clientId() == null || body.clientId().isBlank()) {
                              return writeErrorResponse(
                                  exchange, HttpStatus.BAD_GATEWAY, "oauth_facebook_authorize_failed");
                            }
                            String facebookAuthorizeUrl =
                                UriComponentsBuilder.fromUriString(FACEBOOK_AUTHORIZE_URL)
                                    ...
                                    .encode()
                                    .toUriString();
                            return redirect(exchange, facebookAuthorizeUrl);
                          });
                }
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_facebook_authorize_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
                                ...
                                err == null || err.error() == null || err.error().isBlank()
                                    ? "oauth_facebook_authorize_failed"
                                    : err.error()));
              })
          .onErrorResume(
              WebClientRequestException.class,
              ex -> writeErrorResponse(exchange, HttpStatus.BAD_GATEWAY, "oauth_facebook_authorize_failed"))
          .onErrorResume(
  ...
  private record InternalFacebookAuthorizeRequest(
      UUID projectId, String redirectUri, String state, String codeVerifier) {}

  private record InternalFacebookAuthorizeResponse(String clientId) {}
