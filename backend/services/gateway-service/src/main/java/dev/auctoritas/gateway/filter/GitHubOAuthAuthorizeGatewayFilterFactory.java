...
@Component
public class GitHubOAuthAuthorizeGatewayFilterFactory
    extends AbstractGatewayFilterFactory<GitHubOAuthAuthorizeGatewayFilterFactory.Config> {
  ...
  public GitHubOAuthAuthorizeGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalGitHubAuthorizeRequest request =
          new InternalGitHubAuthorizeRequest(projectId, appRedirectUri, state, codeVerifier);

      return webClient
          .post()
          .uri("/internal/oauth/github/authorize")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalGitHubAuthorizeResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null || body.clientId() == null || body.clientId().isBlank()) {
                              return writeErrorResponse(
                                  exchange, HttpStatus.BAD_GATEWAY, "oauth_github_authorize_failed");
                            }
                            String githubAuthorizeUrl =
                                UriComponentsBuilder.fromUriString(GITHUB_AUTHORIZE_URL)
                                    ...
                                    .encode()
                                    .toUriString();
                            return redirect(exchange, githubAuthorizeUrl);
                          });
                }
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_github_authorize_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
                                ...
                                err == null || err.error() == null || err.error().isBlank()
                                    ? "oauth_github_authorize_failed"
                                    : err.error()));
              })
          .onErrorResume(
              WebClientRequestException.class,
              ex -> writeErrorResponse(exchange, HttpStatus.BAD_GATEWAY, "oauth_github_authorize_failed"))
          .onErrorResume(
  ...
  private record InternalGitHubAuthorizeRequest(
      UUID projectId, String redirectUri, String state, String codeVerifier) {}

  private record InternalGitHubAuthorizeResponse(String clientId) {}
