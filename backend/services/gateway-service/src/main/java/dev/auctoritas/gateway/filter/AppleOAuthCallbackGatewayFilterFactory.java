...
@Component
public class AppleOAuthCallbackGatewayFilterFactory
    extends AbstractGatewayFilterFactory<AppleOAuthCallbackGatewayFilterFactory.Config> {
  private static final String CALLBACK_PATH = "/api/v1/auth/oauth/apple/callback";

  private final WebClient webClient;

  public AppleOAuthCallbackGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalAppleCallbackRequest request =
          new InternalAppleCallbackRequest(code.trim(), state.trim(), callbackUri);

      return webClient
          .post()
          .uri("/internal/oauth/apple/callback")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalAppleCallbackResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null || body.redirectUrl() == null || body.redirectUrl().isBlank()) {
                              return writeErrorResponse(
                                  exchange, HttpStatus.BAD_GATEWAY, "oauth_apple_callback_failed");
                            }
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_apple_callback_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
                                ...
                                err == null || err.error() == null || err.error().isBlank()
                                    ? "oauth_apple_callback_failed"
                                    : err.error()));
              })
          .onErrorResume(
              WebClientRequestException.class,
              ex -> writeErrorResponse(exchange, HttpStatus.BAD_GATEWAY, "oauth_apple_callback_failed"))
          .onErrorResume(
  ...
  private record InternalAppleCallbackRequest(String code, String state, String callbackUri) {}

  private record InternalAppleCallbackResponse(String redirectUrl) {}
