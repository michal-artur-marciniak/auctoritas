...
@Component
public class FacebookOAuthCallbackGatewayFilterFactory
    extends AbstractGatewayFilterFactory<FacebookOAuthCallbackGatewayFilterFactory.Config> {
  private static final String CALLBACK_PATH = "/api/v1/auth/oauth/facebook/callback";

  private final WebClient webClient;

  public FacebookOAuthCallbackGatewayFilterFactory(
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      InternalFacebookCallbackRequest request =
          new InternalFacebookCallbackRequest(code.trim(), state.trim(), callbackUri);

      return webClient
          .post()
          .uri("/internal/oauth/facebook/callback")
          ...
          .exchangeToMono(
              response -> {
                ...
                if (status != null && status.is2xxSuccessful()) {
                  return response
                      .bodyToMono(InternalFacebookCallbackResponse.class)
                      .flatMap(
                          body -> {
                            if (body == null || body.redirectUrl() == null || body.redirectUrl().isBlank()) {
                              return writeErrorResponse(
                                  exchange, HttpStatus.BAD_GATEWAY, "oauth_facebook_callback_failed");
                            }
                ...
                return response
                    ...
                    .defaultIfEmpty(new ErrorResponse("oauth_facebook_callback_failed"))
                    .flatMap(
                        err ->
                            writeErrorResponse(
                                exchange,
                                ...
                                err == null || err.error() == null || err.error().isBlank()
                                    ? "oauth_facebook_callback_failed"
                                    : err.error()));
              })
          .onErrorResume(
              WebClientRequestException.class,
              ex -> writeErrorResponse(exchange, HttpStatus.BAD_GATEWAY, "oauth_facebook_callback_failed"))
          .onErrorResume(
  ...
  private record InternalFacebookCallbackRequest(String code, String state, String callbackUri) {}

  private record InternalFacebookCallbackResponse(String redirectUrl) {}
